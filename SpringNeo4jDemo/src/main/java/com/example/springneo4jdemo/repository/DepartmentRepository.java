...
public interface DepartmentRepository extends Neo4jRepository<Department, UUID> {
    ...
    @Query("match(department:Department{department_name:$departmentName})<-[rel:DEPARTMENT_TAKEN]-(college:College{collegeName:$collegeName})\n" +
            "return department")
    public Department findDepartmentByCollege(String departmentName, String collegeName);

    @Query("with $subjectList as subjectList\n" +
            "match (department:Department{department_name:$departmentName})\n" +
            "match(subject:Subject) where subject.subject_name in   subjectList with collect(subject) as sub,department\n" +
